...
import { Briefcase, Users, BarChart3, Zap } from 'lucide-react';
    ...
    <section id="proposito" className="py-24 md:py-36 bg-slate-50">
      <div ref={ref} className={`container mx-auto px-6 text-center max-w-4xl scroll-fade-up ${isVisible ? 'visible' : ''}`}>
        ...
        <p className="font-heading text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold text-slate-900 leading-relaxed mb-6">
          ...
          conhecimento de negócios e de gestão, adaptação ao{' '}
        ...
        <p className="text-slate-500 text-base sm:text-lg md:text-xl italic mb-16 max-w-3xl mx-auto">
          "Somos o elo que conecta conhecimento, estratégia e oportunidades no setor veterinário."
        </p>

        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 stagger-children ${isVisible ? 'visible' : ''}`}>
          {[
            { icon: Briefcase, label: 'Desenvolvimento de Negócios', desc: 'Projetos de desenvolvimento e crescimento empresarial' },
            { icon: Users, label: 'Consultoria e Assessoria', desc: 'Serviços de consultoria e assessoria organizacional' },
            { icon: BarChart3, label: 'Ferramentas de Inteligência', desc: 'Desenvolvimento de ferramentas de inteligência de dados' },
            { icon: Zap, label: 'Networking de Impacto', desc: 'Eventos de networking de alto impacto' },
          ].map((item) => (
            <div key={item.label} className="flex flex-col items-center gap-4 group p-6 rounded-xl transition-all duration-200 hover:bg-white hover:shadow-md">
